...
describe("mastermind reducer", () => {
  it("creates the initial intro state", () => {
    ...
    expect(state.board[0].pegs).toEqual([
      "select",
      "select",
      "select",
      "select",
    ...
    expect(state.secretCode.every((color) => PEG_COLORS.includes(color))).toBe(
      true,
    );
  });
  ...
  it("starts a game with a randomized secret code", () => {
    ...
    expect(
      playingState.secretCode.every((color) => PEG_COLORS.includes(color)),
    ).toBe(true);
  });
  ...
  it("confirms a secret and places the computer first guess", () => {
    ...
    const state = reducer(
      reducer(initState(), startAlgorithm()),
  ...
  it("gives red, white, and empty feedback for a submitted row", () => {
    ...
    expect(feedbackState.board[0].feedback).toEqual([
      "red",
      "red",
      "white",
      "none",
  ...
  it("makes the next row active and selectable after a non-winning submit", () => {
    ...
    expect(nextState.board[1].pegs).toEqual([
      "select",
      "select",
      "select",
      "select",
    ]);
    expect(nextState.board[0].pegs).toEqual([
      "yellow",
      "blue",
      "silver",
      "pink",
  ...
  it("loses when the last row does not match the secret code", () => {
    ...
    expect(lostState.board[lastRow].feedback).toEqual([
      "none",
      "none",
      "none",
      "none",
  ...
  it("resets game progress and returns to the intro", () => {
    const playingState = reducer(
      reducer(initState(), startGame()),
      submitRow(),
    );
    ...
    expect(resetState.board[0].pegs).toEqual([
      "select",
      "select",
      "select",
      "select",
