                ...
                {Array.from({length: FEEDBACK_LENGTH}, (_, index) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            {peg === undefined || peg === 'none' ? <SmallFeedbackHole /> : null}
                        </div>
                    );
                })}
            </div>
            <div className="picker-box">
                <button type="button" className="picker-color" aria-label="red" onClick={() => place('red')}>
                    <SmallFeedbackPeg type="red" />
                </button>
                <button type="button" className="picker-color" aria-label="white" onClick={() => place('white')}>
                    <SmallFeedbackPeg type="white" />
                </button>
                <button type="button" className="picker-color" aria-label="no more pegs" onClick={() => finalize(entered)}>
                    <span style={{color: '#c00', fontSize: '1.8em', lineHeight: 1}}>✗</span>
                </button>
            </div>
            {error ? <p>That doesn&apos;t match your secret — score it again.</p> : null}
        </div>
    );
};
