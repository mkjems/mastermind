...
export const showColorPicker = (id: number): ShowColorPickerAction => ({
  type: SHOW_COLOR_PICKER,
  id,
});
export const chooseColorAndAdvance = (
  name: Color,
): ChooseColorAndAdvanceAction => ({
...
export const startAlgorithm = (): StartAlgorithmAction => ({
  type: START_ALGORITHM,
});
export const confirmSecret = (secret: Color[]): ConfirmSecretAction => ({
  type: CONFIRM_SECRET,
  secret,
});
